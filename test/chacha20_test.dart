...
void main() {
  for (int i = 0; i < blockFunctionVectors.length; ++i) {
    final ChaCha20TestVector vector = blockFunctionVectors[i];
    test('The ChaCha20 Block Functions Test Vector ${(i + 1)}', () {
      final ChaCha20 chacha20 = ChaCha20(
        key: Uint8List.fromList(vector.key),
        nonce: Uint8List.fromList(vector.nonce),
        counter: vector.counter,
      );

      final Uint8List keystream = chacha20.chacha20Block();

      expect(keystream.length, equals(64));
      expect(keystream, equals(vector.keyStream!));
    });
  }

  for (int i = 0; i < encryptionVectors.length; ++i) {
    final ChaCha20TestVector vector = encryptionVectors[i];
    test('The ChaCha20 Encryption Test Vector ${(i + 1)}', () {
      ...
      final Uint8List plaintext = Uint8List.fromList(vector.plaintext!);
      final Uint8List ciphertext = Uint8List.fromList(vector.ciphertext!);
  ...
  for (int i = 0; i < encryptionVectors.length; ++i) {
    ...
    test('The ChaCha20 Stream Encryption Test Vector ${(i + 1)}', () async {
      ...
      expect(plaintext.length, equals(ciphertext.length));
      expect(plaintext != result, isTrue);
      expect(result, equals(ciphertext));
    });
  }
...
/// A container for ChaCha20 test vectors from RFC 8439.
final class ChaCha20TestVector {
  const ChaCha20TestVector({
  ...
  final int counter;
  final List<int> key, nonce;
  final List<int>? keyStream, plaintext, ciphertext;
}

/// Test vectors for raw ChaCha20 block generation from RFC 8439.
const List<ChaCha20TestVector> blockFunctionVectors = <ChaCha20TestVector>[
  // Test Vector #1
  ChaCha20TestVector(
    key: <int>[
...
/// Test vectors for ChaCha20 encryption from RFC 8439.
const List<ChaCha20TestVector> encryptionVectors = <ChaCha20TestVector>[
  // Test Vector #0
  ChaCha20TestVector(
    key: <int>[
